...
void HttpClient::SendReq(const std::string &url, ReqCallback req_callback, char *post_data, int data_len)
{
	// 给回调函数赋值
	s_req_callback = req_callback;
	mg_mgr mgr;
	mg_mgr_init(&mgr, NULL);
    auto connection = mg_connect_http(&mgr, OnHttpEvent, url.c_str(), NULL, post_data, data_len);
