...
void HttpServer::HandleHttpEvent(mg_connection *connection, http_message *http_req)
{
    ...
    const char *body = http_req->body.p;
    int body_len = http_req->body.len;
    QByteArray ba = QByteArray(body, body_len);
	auto it = s_handler_map.find(url);
	if (it != s_handler_map.end())
	{
		ReqHandler handle_func = it->second;
        handle_func(url, body, body_len, connection, &HttpServer::SendHttpRsp);
	}
